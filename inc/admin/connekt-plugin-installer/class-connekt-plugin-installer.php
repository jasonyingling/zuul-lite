   ...
   class Connekt_Plugin_Installer {
         ...
         <?php
            require_once( ABSPATH . 'wp-admin/includes/plugin-install.php' );
   		   ...
   		   foreach($plugins as $plugin) :
               ...
               $button_text = __('Install Now', 'zuul-lite');
               ...
               //echo '</pre>';
	               ...
	               if(self::check_file_extension($main_plugin_file)){ // check file extension
	   	            if(is_plugin_active($main_plugin_file)){
	                  	...
	                  	$button_text = __('Activated', 'zuul-lite');
	                  } else {
	                     // It's installed, let's activate it
	                  	$button_classes = 'activate button button-primary';
	                  	$button_text = __('Activate', 'zuul-lite');
	                  }
	               }
      ...
      public static function render_template($plugin, $api, $button_text, $button_classes){
         ...
         <div class="plugin">
		      <div class="plugin-wrap">
               ...
               <p class="plugin-author"><?php _e('By', 'zuul-lite'); ?> <?php echo $api->author; ?></p>
			   </div>
      ...
      */
		public function cnkt_plugin_installer(){
			...
			if ( ! current_user_can('install_plugins') )
				wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'zuul-lite' ) );
			...
			if (! wp_verify_nonce( $nonce, 'cnkt_installer_nonce' ))
				wp_die( __( 'Error - unable to verify nonce, please try again.', 'zuul-lite') );
      ...
      */
		public function cnkt_plugin_activation(){
			if ( ! current_user_can('install_plugins') )
				wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'zuul-lite' ) );
			...
			// Check our nonce, if they don't match then bounce!
			if (! wp_verify_nonce( $nonce, 'cnkt_installer_nonce' ))
				die( __( 'Error - unable to verify nonce, please try again.', 'zuul-lite' ) );
      ...
      public function enqueue_scripts(){
         wp_enqueue_script( 'plugin-installer', CNKT_INSTALLER_PATH. 'assets/installer.js', array( 'jquery' ));
			wp_localize_script( 'plugin-installer', 'cnkt_installer_localize', array(
               ...
               'install_now' => __('Are you sure you want to install this plugin?', 'zuul-lite'),
               'install_btn' => __('Install Now', 'zuul-lite'),
               'activate_btn' => __('Activate', 'zuul-lite'),
               'installed_btn' => __('Activated', 'zuul-lite')
            ));
